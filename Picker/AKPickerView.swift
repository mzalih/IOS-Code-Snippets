...
private class AKCollectionViewCell: UICollectionViewCell {
    ...
    func initialize() {
        ...
        let frame = CGRect(x: self.contentView.bounds.origin.x , y: self.contentView.bounds.origin.y + (self.contentView.bounds.size.height/2), width:  self.contentView.bounds.size.width, height:  self.contentView.bounds.size.height/2);
        ...
        let frameIcon = CGRect(x: self.contentView.bounds.origin.x , y: self.contentView.bounds.origin.y , width: self.contentView.bounds.size.width, height:  self.contentView.bounds.size.height);
        
        self.imageView = UIImageView(frame: frameIcon)
...
*/
@IBDesignable
public class AKPickerView: UIView, AKPickerViewDataSource,AKPickerViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, AKCollectionViewLayoutDelegate {
    
    var changeSelection :((index:Int) -> Void)?;
    var startPoint:Int = 0
    var endPoint:Int   = 0
    
    public func setChanger(onChange:((index:Int) -> Void)){
        changeSelection = onChange;
    }
    public func pickerView(pickerView: AKPickerView, didSelectItem item: Int){
        if(changeSelection != nil){
            changeSelection!(index: item);
        }
    }
    public func numberOfItemsInPickerView(pickerView: AKPickerView) -> Int{
        return (endPoint - startPoint)*2 + 2;
    }
    public func pickerView(pickerView: AKPickerView, titleForItem item: Int) -> String{
        
        if((item % 2) != 0){
            return  " "
        }
        return (startPoint + (item/2)).description;
    }
    
    public func pickerView(pickerView: AKPickerView, imageForItem item: Int) -> UIImage{
        return UIImage(named: "sep")!
    }
    
    
    @IBInspectable var newStartPoint: Int = 0   {
        didSet{
            startPoint = newStartPoint
        }
        
    }
    @IBInspectable var newEndPoint: Int = 0   {
        didSet{
    ...
    */
    ...
    public func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        if let image = self.dataSource?.pickerView?(self, imageForItem: indexPath.item) {
